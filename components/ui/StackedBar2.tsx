      ...
      {rows.map((row, i) => {
        ...
        return (
          <div key={row.label} className="flex items-center gap-3 text-[13.5px]">
            ...
            <span className="flex h-[18px] flex-1 overflow-hidden rounded-[var(--r-sm)]" style={{ background: "var(--surface-sunken)" }}>
              <span
              ...
              <span
                className="bar-grow block h-full"
                style={{ width: `${100 - aPct}%`, background: "#dddddd", animationDelay: `${i * 60 + 40}ms` }}
              />
            </span>
            <span className="w-[96px] shrink-0 text-right font-semibold text-[var(--ink-secondary)]">
              {row.displayA} &middot; {row.displayB}
            </span>
          </div>
        );
      })}
      <div className="mt-1 flex items-center gap-4 text-[12.5px] font-semibold text-[var(--ink-secondary)]">
        <span className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full" style={{ background: "#003ae3" }} />
          {aLabel}
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full" style={{ background: "#dddddd" }} />
          {bLabel}
        </span>
      </div>
    </div>

      ...
      {items.map((item, i) => {
        ...
        return (
          <div key={item.label} className="flex items-center gap-3 text-[14px]">
            <span className="w-[134px] shrink-0 font-semibold text-[var(--ink-secondary)]">{item.label}</span>
            <span className="h-[18px] flex-1 overflow-hidden rounded-[var(--r-sm)]" style={{ background: "var(--surface-sunken)" }}>
              <span
                className="bar-grow block h-full rounded-[var(--r-sm)]"
                style={{ width: `${pct}%`, background: step.bg, animationDelay: `${i * 60}ms` }}
              />
            </span>
